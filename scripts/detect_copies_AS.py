...
def detect_copies(bam):
    ...
    align_start = 152303642  # Start coordinate for required read span (For capturing reads that span exon 3 enough for both INS regions)
    align_stop = 152307641  # End coordinate for required read span (For capturing reads that span exon 3 enough for both INS regions)
    ...
    read_reps, fasta_dict, qual_dict, read_rep_coords = parse_sam(bam, min_size, start, stop, align_start, align_stop)
...
def parse_sam(bam, min_size, start, stop, align_start, align_stop):
    read_reps = {}
    read_rep_coords = {}
    ...
    for seg in sam:
        ...
        if cigar:
            if alignment_within(rcoord_start, rcoord_end, align_start, align_stop):
                read_reps[seg.query_name], read_rep_coords[seg.query_name] = parse_cigar(cigar, rcoord_start, min_size, start, stop)
        else:
...
# Additional filter for adaptive sampling data to capture only reads that span exon 3 enough for both INS regions
