...
mkdir new_analysis_050822 && cd new_analysis_050822
...
```bash
# Created a bash script containing these commands
"
#!/bin/bash
...
for file in /apps-data/202202_filaggrin_AS/Nanopore_FLG_amplicon_seq_fast5/new_analysis_050822/bam_hg38/*.mm-hg38.bam; do
filename="${file##*/}"
...
while read f;
    do 
        ...
        if [[ "$G" -eq 1 ]]; then
            H=`grep 'yes' ./bam_hg38/${f}_F2308_q60_12000_headered.sorted.fl100sr.summmary.tsv | cut -f 1`
...
while read f;
    do
        ...
        BASE="/apps-data/202202_filaggrin_AS/Nanopore_FLG_amplicon_seq_fast5/new_analysis_021222"
...
while read f;
    do
        ...
        if [ -f ./pmdv/${f}"_11rep.vcf.gz" ]; then
            ...
            bcftools view -H ./pmdv/${f}"_11rep.vcf.gz" | awk -F'\t' '{ if ($2 < 6498) $2=$2+152300000; else if ($2 >= 6498) $2=$2+152299028} 1' OFS='\t'  > temp_vcf.txt
            cat temp_header.txt temp_vcf.txt | bcftools sort - | perl -pe 's/FLG_hg38_11rep_5526_6497/chr1/g' > ./liftover/${f}_11rep.vcf
        ...
        if [ -f ./pmdv/${f}"_12rep.vcf.gz" ]; then
            ...
            bcftools view -H ./pmdv/${f}"_12rep.vcf.gz" | awk -F'\t' '{ if ($2 < 6498) $2=$2+152300000; else if ($2 >= 6498) $2=$2+152299028} 1' OFS='\t' > temp_vcf.txt
            cat temp_header.txt temp_vcf.txt | bcftools sort - | perl -pe 's/FLG_hg38_11rep_5526_6497/chr1/g' > ./liftover/${f}_12rep.vcf
